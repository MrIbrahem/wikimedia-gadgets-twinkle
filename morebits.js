...
(function () {
	...
	Morebits.quickForm.element.prototype.compute = function QuickFormElementCompute(data, in_id) {
		...
		switch (data.type) {
			...
			case 'select':
				...
				if (data.disabled) {
					select.setAttribute('disabled', 'معطل');
				}
			...
			case 'option':
				...
				if (data.selected) {
					node.setAttribute('selected', 'حُدد');
				}
				if (data.disabled) {
					node.setAttribute('disabled', 'معطل');
				}
			...
			case 'radio':
				...
				if (data.list) {
					for (i = 0; i < data.list.length; ++i) {
						...
						if (current.checked) {
							subnode.setAttribute('checked', 'محدد');
						}
						if (current.disabled) {
							subnode.setAttribute('disabled', 'معطل');
						}
						...
						if (current.subgroup) {
							...
							subnode.addEventListener('تغيير', event, true);
						}
						// add users' event last, so it can interact with the subgroup
						if (data.event) {
							subnode.addEventListener('تغيير', data.event, false);
						} else if (current.event) {
							subnode.addEventListener('تغيير', current.event, true);
						}
					}
				}
	...
	Morebits.date.localeData = {
		months: [msg('january', 'يناير'), msg('february', 'فبراير'), msg('march', 'مارس'),
		msg('april', 'أبريل'), msg('may_long', 'مايو'), msg('june', 'يونيو'),
		msg('july', 'يوليو'), msg('august', 'أغسطس'), msg('september', 'سبتمبر'),
		msg('october', 'أكتوبر'), msg('november', 'نوفمبر'), msg('december', 'ديسمبر')],
		monthsShort: [msg('jan', 'يناير'), msg('feb', 'فبراير'), msg('mar', 'مارس'),
		msg('apr', 'أبريل'), msg('may', 'مايو'), msg('jun', 'يونيو'),
		msg('jul', 'يوليو'), msg('aug', 'أغسطس'), msg('sep', 'سبتمبر'),
		msg('oct', 'أكتوبر'), msg('nov', 'نوفمبر'), msg('dec', 'ديسمبر')],
		days: [msg('sunday', 'الأحد'), msg('monday', 'الاثنين'), msg('tuesday', 'الثلاثاء'),
		msg('wednesday', 'الأربعاء'), msg('thursday', 'الخميس'), msg('friday', 'الجمعة'),
		msg('saturday', 'السبت')],
		daysShort: [msg('sun', 'أحد'), msg('mon', 'اثنين'), msg('tue', 'ثلاثاء'),
		msg('wed', 'أربعاء'), msg('thu', 'خميس'), msg('fri', 'جمعة'),
		msg('sat', 'سبت')],
		...
		var fnCheckPageName = function (response, onFailure) {
			...
			if (page) {
				...
				if (response.redirects) {
					...
					new Morebits.status('ملاحظة', msg('redirected', ctx.pageName, resolvedName, 'تم ترك تحويلة من ' + ctx.pageName + ' إلى ' + resolvedName));
				}
	...
	Morebits.batchOperation = function (currentAction) {
		...
		var fnDoneOne = function () {
			...
			if (ctx.countFinished < total) {
				...
				ctx.statusElement.status(msg('النسبة المئوية', progress, progress + '%'));
