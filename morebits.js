...
(function () {
	...
	Morebits.wiki.page = function (pageName, status) {
		...
		this.load = function (onSuccess, onFailure) {
			...
			if (!onSuccess) {
				ctx.statusElement.error('خطأ داخلي: لم تُوفر وظيفة رد الاتصال onSuccess لـ load()!');
		...
		this.save = function (onSuccess, onFailure) {
			...
			if (!ctx.pageLoaded && !canUseMwUserToken) {
				ctx.statusElement.error('خطأ داخلي: لم يُعين معرف المراجعة للتراجع قبل تنفيذ التراجع!');
			...
			if (!ctx.editSummary) {
				...
				if (ctx.editMode === 'new' && ctx.newSectionTitle) {
					...
					ctx.statusElement.error('خطأ داخلي: لم يُعين ملخص التعديل قبل الحفظ!');
					ctx.onSaveFailure(this);
					return;
				}
			}
		...
		this.lookupCreation = function (onSuccess, onFailure) {
			...
			if (!onSuccess) {
				ctx.statusElement.error('خطأ داخلي: لم تُقدم دالة onSuccess callback في lookupCreation()!');
		...
		this.move = function (onSuccess, onFailure) {
			...
			if (!ctx.moveDestination) {
				ctx.statusElement.error('خطأ داخلي: لم يُعين اسم الصفحة الوجهة قبل النقل!');
		...
		var fnLookupNonRedirectCreator = function () {
			...
			if (!ctx.creator) {
				...
				if (!ctx.creator) {
					ctx.statusElement.error('لم يُعثر على اسم منشئ الصفحة');
					ctx.onLookupCreationFailure(this);
					return;
				}
			...
			if (!ctx.timestamp) {
				ctx.statusElement.error('لم يُعثر على الطابع الزمني لإنشاء الصفحة');
				ctx.onLookupCreationFailure(this);
				return;
			}
